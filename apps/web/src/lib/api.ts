...
function getBase(): string {
  const url = process.env['SEARCH_REC_URL'];
  if (!url) throw new Error('SEARCH_REC_URL is not set');
  return url;
}
...
export function getRecommendHome(userId: string, topK = 10): Promise<RecommendResponse> {
  ...
  return fetchJSON(`${getBase()}/recommend/home?${qs}`, RecommendResponseSchema);
}
...
export function getRecommendRelated(productId: number, topK = 6): Promise<RecommendResponse> {
  ...
  return fetchJSON(`${getBase()}/recommend/related?${qs}`, RecommendResponseSchema);
}
...
export function searchProducts(query: string, topK = 20): Promise<SearchResponse> {
  ...
  return fetchJSON(`${getBase()}/search?${qs}`, SearchResponseSchema);
}
